...
C         DO 280 K=1,NLAY
C         KK=K
C         CALL U2DINT(IBOUND(:,:,KK),ANAME(1),NROW,NCOL,KK,INBAS,IOUT)
C  280    CONTINUE
C      ELSE
C         CALL U2DINT(IBOUND(:,:,1),ANAME(1),NLAY,NCOL,-1,INBAS,IOUT)
C      END IF
C
C8F-----READ AND PRINT HEAD VALUE TO BE PRINTED FOR NO-FLOW CELLS.
C      IF(IFREFM.EQ.0) THEN
C         READ(INBAS,'(F10.0)') HNOFLO
C      ELSE
C         READ(INBAS,*) HNOFLO
C      END IF
C      HNF=HNOFLO
C      WRITE(IOUT,3) HNOFLO
C    3 FORMAT(1X,/1X,'AQUIFER HEAD WILL BE SET TO ',1PG12.5,
C     1       ' AT ALL NO-FLOW NODES (IBOUND=0).')
C
...
C         DO 300 K=1,NLAY
C         KK=K
C         CALL U2DREL(STRT(:,:,KK),ANAME(2),NROW,NCOL,KK,INBAS,IOUT)
C  300    CONTINUE
C      ELSE
C         CALL U2DREL(STRT(:,:,1),ANAME(2),NLAY,NCOL,-1,INBAS,IOUT)
C      END IF
C
...
C      DO 400 I=1,NROW
C      DO 400 J=1,NCOL
C      HNEW(J,I,K)=STRT(J,I,K)
C      IF(IBOUND(J,I,K).EQ.0) HNEW(J,I,K)=HNF
C  400 CONTINUE
C

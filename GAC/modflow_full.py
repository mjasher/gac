...
for IPER in range(NPER):
	for ISTP in range(NSTP[IPER]):
		...
		# 	# if convertible
		# 	if laytyp[k] != 0:
		# 		SGWF2LPF7HCOND(KK,KITER,KSTP,KPER)
		# 	if k < nlay-1 
		# 		if laytyp[k] != 0 or laytyp[k+1] != 0:
		# 			SGWF2LPF7VCOND(KK)
		...
		# 					if ibound[i,j,k] >= 0:
		# 						RHO=SC1[i,j,k]*TLED
		# 						HCOF[i,j,k]=HCOF[i,j,k]-RHO
		# 						RHS[i,j,k]=RHS[i,j,k]-RHO*HOLD[i,j,k]
		# 		else:
		# 			for i in range(0, nrow):
		# 				for j in range(0, ncol):
		...
		for k in range(0, nlay):
			for i in range(0, nrow):
				for j in range(0, ncol):
					...
					elif TOP[i,j,k] > h_old[node] and h_old[node] > BOT[i,j,k]:
					...
					HCOF[i,j,k] -= SCB / dt 
					RHS[i,j,k] += (SCA * ( TOP[i,j,k] - h_old[i,j,k] ) - SCB * TOP[i,j,k]) / dt
